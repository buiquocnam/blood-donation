...
export function EventsList({ events = [], isLoading, onSearch, onFilterStatus, role = "user" }: EventsListProps) {
  const [localSearch, setLocalSearch] = useState("");
  const [localFilter, setLocalFilter] = useState<string>("all");
  const [isSearchExpanded, setIsSearchExpanded] = useState(false);
  
  // Sử dụng debounce cho search query
  ...
  // Gọi onSearch khi debouncedSearch thay đổi
  useEffect(() => {
    if (onSearch) {
    ...
    <div>
      ...
      {filteredEvents.length === 0 ? (
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="grid grid-cols-1 gap-6 px-0"
        >
          {filteredEvents.map((event, index) => (
            <motion.div key={event.IdSuKien || `event-${index}`} variants={item} className="w-full">
