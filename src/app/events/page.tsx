    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex gap-2 items-center">
              <span className="text-foreground">Lọc theo:</span>
            ...
            <div className="relative">
              <input 
                ...
                className="border border-border rounded-md px-3 py-1.5 pl-8 w-full md:w-64 bg-background"
              />
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 absolute left-2.5 top-2.5 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid gap-8">
          {events.map((event) => (
            <div key={event.id} className="bg-card rounded-lg shadow-md overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="md:w-2/3 p-6">
                <div className="flex justify-between items-start mb-2">
                  <Link href={`/events/${event.id}`}>
                    <h2 className="text-xl font-bold hover:text-primary transition-colors duration-fast">{event.title}</h2>
                  </Link>
                  <span className={`text-sm font-medium px-2 py-1 rounded ${
                    event.status === "Đang diễn ra" ? "bg-success/10 text-success" : "bg-secondary/10 text-secondary"
                  }`}>
                    {event.status}
                  </span>
                </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-muted-foreground mb-4">Không tìm thấy sự kiện phù hợp với bạn?</p>
          <Button variant="outline" asChild>
